...
class ControllerExtensionModuleOTBlog extends Controller {
	public function index($setting) {
		...
		$data['all_blogs'] = $this->url->link('information/ot_blog/blogs');
		...
		foreach ($this->model_ontheme_otblog->getBlogsByModule($setting['module_id'], $setting['limit']) as $result) {			
			...
			$data['blogs'][] = array(
				...
				'href'        => $this->url->link('information/ot_blog', 'ot_blog_id=' . $result['ot_blog_id'])
			);  	
		}
		
				
		return $this->load->view('extension/module/ot_blog', $data); 
	}
}