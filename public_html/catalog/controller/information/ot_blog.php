<?php
class ControllerInformationOTBlog extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('information/ot_blog');
		$this->load->model('ontheme/otblog');
		$this->load->model('tool/image');

		if (!isset($this->request->get['ot_blog_id'])) {
			$this->response->redirect($this->url->link('information/ot_blog/blogs'));
		}

		$language = $this->model_ontheme_otblog->getLanguageByCode($this->session->data['language']);

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_ontheme_otblog->addComment($this->request->get['ot_blog_id'], $this->request->post, $language['language_id']);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('information/ot_blog', '', true)
		);

		$data['breadcrumbs'][] = array(
			'text' => $blog_info['title'],
			'href' => $this->url->link('information/ot_blog', 'ot_blog_id=' . $this->request->get['ot_blog_id'], true)
		);

		$data['heading_title'] = $blog_info['title'];
		$data['date'] = date($this->language->get('date_format_short'), strtotime($blog_info['date_added']));
		
		$module = $this->model_ontheme_otblog->getModule($blog_info['module_id']);
		...
		$blog_comments = $this->model_ontheme_otblog->getBlogComments($this->request->get['ot_blog_id'], $language['language_id']);
		...
		$data['action'] = $this->url->link('information/ot_blog', 'ot_blog_id=' . $this->request->get['ot_blog_id'], true);
		...
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('information/ot_blog', 'ot_blog_id=' . $this->request->get['ot_blog_id'], true);
		}
		...
		$this->response->setOutput($this->load->view('information/ot_blog', $data));
		}else {
			$url = '';
	...
	public function blogs() {
		...
		$blogs = $this->model_ontheme_otblog->getBlogs();

		foreach ($blogs as $blog) {
			$module_blog_data = array();

			$module = $this->model_ontheme_otblog->getModule($blog['module_id']);

			$setting = json_decode($module['setting'],true);

			if (!empty($setting['status']) && $setting['status']) {
				$module_blogs = $this->model_ontheme_otblog->getBlogsByModule($blog['module_id']);
			
			
				foreach ($module_blogs as $module_blog) {
				
				$total_comments = $this->model_ontheme_otblog->getTotalBlogComments($module_blog['ot_blog_id']);
					...
					$module_blog_data[] = array(
						...
						'href'        => $this->url->link('information/ot_blog', 'ot_blog_id=' . $module_blog['ot_blog_id'], true)
					);
				}
		...
		$this->response->setOutput($this->load->view('information/ot_blog_blogs', $data));
	}
}