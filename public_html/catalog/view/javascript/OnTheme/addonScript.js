...
$(document).ready(function() {
        ...
        $('#formInput49-2, #formInput50-3').attr('placeholder', '+7 (978) 000-00-00');
		// $('#formInput50-1').attr('disabled', 'true');
		...
		//Заполнение поля имени товара для заказа в 1 клик

		let productName = document.querySelector('h1.product-name');

		if(productName){
			$('#formInput50-1').attr('value', productName.innerText);
		}

        $('.one-click-button').on('click', function(){
            $('.one-click-form-overlay').fadeIn();
        })
        
        $('.one-click-form-overlay').on('click', function(e){
            if(e.target.classList.contains('one-click-form-overlay')){
